...
export default function Peminjaman() {
  ...
  const myLoans = loans.filter(
    (t) => t.status === "ACTIVE" && String(t.user_id).includes(userId),
  );
  const myHistory = loans.filter(
    (t) => t.status === "RETURNED" && String(t.user_id).includes(userId),
  );
            ...
            {activeRows.map((t) => {
              ...
              return (
                <div
                  ...
                  style={
                    isWarning && !overdue ? { borderColor: "#fde047" } : {}
                  }
                >
                  <div
