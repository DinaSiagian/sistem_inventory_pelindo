...
// Data pekerjaan/anggaran diambil dari database via API (budgetAPI.getProjects)
let mockPekerjaan = [];

// Data aset diambil dari database via API (barangAPI.getAll)
let mockAssets = [];

// Data user diambil dari database via API (userAPI.getAll)
let mockUsers = [];
...
function HistoryPage({ assetCode, assetName, borrows, returns, onBack }) {
  ...
  const [borrows, setBorrows] = useState(() => JSON.parse(sessionStorage.getItem('SWR_UserBorrows')) || []);
      ...
      const [usersState, setUsersState] = useState(() => JSON.parse(sessionStorage.getItem('SWR_UserUsers')) || []);
  ...
  const loadData = async () => {
    try {
      ...
      if (txRes.data && txRes.data.success) {
        let activeUser = { };
      try {activeUser = JSON.parse(localStorage.getItem("user")) || JSON.parse(sessionStorage.getItem("currentUser")) || {}; } catch (e) { }
      ...
      <div style={S.root}>
        ...
        <div style={S.toolbar}>
          ...
          <div style={{
            ...
            Total BAST: <span style={{ color: "#0f172a" }}>{groupedTransactions.length}</span>
          </div>
        </div>
        ...
        <div style={{ ...S.page }}>
          ...
          <div style={{ padding: "1rem", borderTop: "1px solid #e2e8f0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ fontSize: ".84rem", color: "#64748b" }}>
              ...
              <strong>{groupedTransactions.length}</strong> transaksi
            </div>
            <div style={{ display: "flex", gap: ".5rem", alignItems: "center" }}>
              ...
              <div style={{ fontSize: ".84rem", fontWeight: 500, color: "#64748b" }}>
              ...
              <button
                ...
                onClick={() => setCurrentPage((p) => Math.min(Math.ceil(groupedTransactions.length / itemsPerPage) || 1, p + 1))}
              >
                Next
              </button>
            </div>
